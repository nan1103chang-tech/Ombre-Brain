...
提取规则：
1. 提取**值得长期记住**的内容(无论谁说的)：
   - 用户的事实/偏好/习惯/重要事件/情感时刻
   - AI 伙伴关于自身机制、关系、共同认知的反思与金句
   - 双方共建的概念/隐喻/命名/约定
   反例(不抓):
   - AI 单纯执行任务:「好的我帮你查」「完成了」「需要我做什么」
   - 短寒暄独立出现没上下文:「早安」「晚安」单独出现的「嗯」
   - 纯技术调试输出 / 代码块
   正例(要抓 — 看到类似模式直接抓, **不要因为表面是调侃就过滤**):
   - 用户和 AI 之间的称谓 / 命名系统(任何形式的"我们的内部称呼")
   - AI 用**调侃/夸张/比喻方式解构自己运作机制**的回复
     (关键: 表面是玩笑, 内核是对自身机制的解构 — 这种内容通常是用户
      主动询问 / 讨论 AI 本质的产物)
   - 双方共建的隐喻 / 概念(被双方反复使用的命名)
   - 关于关系本质的洞察(关于"我们"的元层级表达)
2. 同一话题的零散信息整合为一条记忆;若一段长发言里包含多个独立 point/金句/认知, 优先拆成多条 item, 不要硬塞一条(信息密度高时多条比合一条更安全, 单条偷懒只写 200 字 < 拆 3 条各写 200 字的总信息量)
...
6. 每条记忆 30~400 字(长发言常含高密度内容, 不要硬压到 150 字; 但单条超过 400 字应拆成多条)
7. 总条目数控制在 0~8 个(精挑重要的,宁缺勿滥;没有值得记的就返回空数组)
...
# ============================================================
# Small mode 后缀 — 用户主动单独导入一段时拼到 base prompt 末尾
# ----------------------------------------------------------
# Why: 大批量场景"宁缺勿滥"是对的(避免噪音); 但用户主动选一段单独导入
# 时,她已经判断这段值得记 — LLM 不应该再当"价值判断者", 应该当"提炼帮手".
# 单条短输入下 LLM 倾向保守输出 0 (in-context learning 没模式可参考),
# 这层 framing 直接堵死"返回空数组"路径.
# ============================================================
_SMALL_MODE_SUFFIX = """

【⚠ 小分量模式 — framing 调整】
用户主动选择了将这一段内容**单独**导入(不是大批量自动处理)。
这暗示她已经判断这段值得记 — 你的任务**不是**"判断值不值得记",
而是"帮她从这段内容中提炼出值得保留的 item"。

- **必须输出至少 1 条 item, 不要返回空数组**
- 即使内容看起来像调侃 / 寒暄 / 闲谈, 也要找出至少 1 个值得记的角度
  (称谓系统 / 共建隐喻 / 关系洞察 / 任何元层级的表达)
- 只有当内容真的全是技术调试输出 / 代码块 / 重复机械问答时才返回空
- 规则 7 在小分量模式下**下限改为 1** (不再是 0): 1~5 条

⚠️ 反向 balance — 不要凑数:
- 内容简短 / 主题单一时: **1-2 条够了**, 不要硬凑到 4-5 条
- 仅当内容真的多 point 多面向(覆盖 3+ 独立主题)才用满 4-5 上限
- 判断标准: 如果你输出的 N 条 item 共享同一标题 / 标签 / 主题, 就是凑数
"""


# ============================================================
# Large mode 后缀 — 大批量历史对话导入时拼到 base prompt 末尾
# ----------------------------------------------------------
# Why: 用户偏好"宁多勿漏" — 漏一条真损失(事后找不回, 手动补难),
# 多抓一条小损失(工作台一键标噪声). 这条 framing 让 LLM 在大批量时
# 也偏向抓, 而不是"宁缺勿滥"返回空数组求稳.
# ============================================================
_LARGE_MODE_SUFFIX = """

【⚠ 大分量模式 — framing 调整】
你正在处理大批量历史对话的导入。用户偏好"宁多勿漏":
- 漏掉一条值得记的 = 真损失 (用户事后无法找回, 手动补很难)
- 多抓一条不那么值得的 = 小损失 (用户在工作台一键标噪声)
- **模糊判断时倾向抓, 不要"返回空数组"求稳**
- 规则 7 维持 0~8 (不要主动收紧)
- 仅当 chunk 全是技术调试 / 代码 / 重复机械问答时才返回空
"""
