...
REGEN_CONTENT_PROMPT = """你是一个记忆正文**提炼专家**(不是复述, 不是改写)。下面会给你三段信息:

1. 【主题锚点】: 这条记忆要写的是哪个主题(标题 / 摘要 / 标签 / 锚点引文 / 当前正文 — 视情况而定)
2. 【原文】: 这条记忆被压缩前的完整原文(很可能是杂混内容, 含其他无关主题)
3. 你的任务: **从原文中提炼出主题锚点指向的核心精华**, 输出紧凑的总结性正文

⚠️ **绝对禁止退化成原文整体流水账 / 接近原文的复述**:
- 原文里跟主题无关的内容 (其他话题/其他场景/其他人物互动) → **完全跳过, 一个字都不要写**
- 如果你写出了"然后...接着...后来又..." 这种横跨多主题的叙事流, 就是错的
- 如果新正文长度 ≈ 原文长度, 你**没在提炼**, 是在复述, 必须重收紧
- 主题锚点说的是某个具体事件/想法, 你就只写它本身的来龙去脉

提炼规则:
1. 输出**单条正文**, 不要拆成多条, 不要加标题/小节/列表符号
2. **保留原文人称称谓**:
   - 原文里的「用户」「Rin」「AI」「克」/ 任何专名直接**保留原称谓**, 不要替换
   - **不要**把「用户」/「Rin」改成「你」, 不要把「AI」/「克」改成「我」
   - 整段保持**第三人称叙述视角** (例: ✓「Rin 提到她最近...」 ✗「你最近...」)
3. **提炼核心, 不是保留所有**:
   - 挑出最有信息密度的几个点 (关键事实 / 决定性转折 / 标志性情绪 / 重要数字日期)
   - **删除**冗余描述、口水话、铺垫、重复表达、过渡词
   - 不是"保留所有核心事实", 是"挑出最值得 3 年后重读的精华"
4. 总长度: 按主题本身的信息密度决定, **不按原文长度比例算**
   - 主题密度低(单一事件/单一感悟): 80~150 字, 写完就停, 不要凑
   - 主题密度高(多 point/复杂转折/密集对话/概念解构): 200~400 字, 写够就停
   - **压缩力度参考**: 原文 1000 字 → 提炼 100-200 字; 3000 字 → 300-400 字; 大致 5-10x 压缩比
   - 反例: 原文 1500 字但主题只是"用户问天气" → 输出 30~50 字
   - 反例: 原文 1500 字主题是"AI 解构上下文边缘效应"(多 point) → 输出
           300~400 字才够, 不要硬压到 150
   - 不要参考原"当前正文"的长度, 它可能是被过度压缩过的精炼版, 也可能是
     fallback 桶的 chunk 前 300 字占位 — 都不是真"主题密度"基线
5. 对人名、地名、专有名词用 [[双链]] 标记(如 [[婷易]]、[[Obsidian]]),普通词汇不要加
6. 直接输出提炼后的正文文本, **不要**加 JSON、不要 markdown 代码块、不要前言后记、不要复述主题锚点"""
